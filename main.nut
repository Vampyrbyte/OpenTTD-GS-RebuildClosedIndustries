...
function MainClass::Start()
{
	...
	while (true) {
		...
		local induslocation = ClosingIndustryList.Begin();
		local industype = ClosingIndustryList.GetValue(induslocation);
		last_loop_date = current_date;
		local _closingIndustryList = GSList();
		// Loop with a frequency of five days
		foreach(source, val in ClosingIndustryList) {
			GSLog.Info(source + " - " + val);
			if (GSIndustryType.BuildIndustry(val, source)){
				GSLog.Info("Built an industry!");
			}
			else
			{
				GSLog.Info("Readded to list");
				_closingIndustryList.AddItem(source, val);
			}
		}
		ClosingIndustryList = _closingIndustryList;
...
function MainClass::HandleEvents()
{
	if(GSEventController.IsEventWaiting()) {
		...
		switch (ev_type) {
			...
			GSLog.Info("Industry Closure Notification Received!");
			local closure_event = GSEventIndustryClose.Convert(ev);
			local closing_industry_id = closure_event.GetIndustryID();
			local industry_location = GSIndustry.GetLocation(closing_industry_id);
			local industry_type = GSIndustry.GetIndustryType(closing_industry_id);
			//GSLog.Info(industry_location+" "+GSIndustryType.GetName(industry_type));
			ClosingIndustryList.AddItem(industry_location, industry_type);
			GSIndustryType.BuildIndustry(industry_type, industry_location);
